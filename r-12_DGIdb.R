...
df1 <- read.csv('Dgidb.csv') %>%
  select(1) %>% dplyr::rename("Var"="gene") %>% 
  bind_rows(read.csv('Dgidb.csv') %>% 
              select(2) %>% dplyr::rename("Var"="drug")) %>% 
...
edges <- read.csv('Dgidb.csv') %>% select(1,2) %>% 
  separate(col=gene,into = c("gene"), sep = " ") %>% 
  separate(col=drug,into = c("drug"), sep = " ") %>% 
...
p <- ggraph::ggraph(graph = tidy_graph, layout = "fr") + 
  ...
  scale_color_manual(values=c("#A6D854","#7570B3"))+
  scale_edge_width(range = c(0.5, 1.5)) +
  scale_size(range = c(5, 10)) + 
  ggraph::theme_graph() +
  theme(legend.position = "none")
p
pdf("01.cluster.pdf", width = 5, height = 5,family = "Times")
print(p)
dev.off()
png("01.cluster.png", width = 5, height = 5,res = 600,units = 'in',family = "Times")
print(p)
dev.off()
